...
# *** EXPLANATION ***
# WE NOW USE A FACTORY TO CREATE GAME OBJECTS SNAKE AND FRUIT.
# *******************
...
class Factory:
    ...
    def createSnake(self, length, color=GREEN, **args):
        ...
        init_pos = self._board.getRandomTile(color=color)
        tiles = [init_pos]
        ...
        for i in range(length - 1):
            ...
            if x < 0:
                x = 0
            if x >= self._board.getNbCols():
                x = self._board.getNbCols() - 1
            if y < 0:
                y = 0
            if y >= self._board.getNbRows():
                y = self._board.getNbRows() - 1
            tiles.append(Tile(col=x, row=y, color=color))

        # Create Snake object
        snake = Snake(self._board, tiles=tiles, direction=snake_dir, **args)
        
        # Add object to board
        self._board.addObject(snake)
...
class Game:
    ...
    def __init__(self, width=WIDTH, height=HEIGHT, tile_size=TILE_SIZE,
        ...
        # Create the factory
        self._factory = Factory(board=self._board)
        self._factory.declareFruit('apple', fruit_color, points=1)
        ...
        self._snake = self._factory.createSnake( length=snake_init_length,
                color=snake_color, gameover_on_exit=gameover_on_exit)

        # Create first fruit
        logger.debug("Create first Fruit instance.")
        self._factory.createFruit('apple')
